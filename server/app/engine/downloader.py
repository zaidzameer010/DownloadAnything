...
def get_app_support_dir() -> Path:
    ...
    app_dir = base_path / "DownloadAnything"
...
def get_aria2_executable_path() -> Optional[Path]:
    ...
    if getattr(sys, "frozen", False):
        ...
        possible_dirs.extend([
            Path(__file__).resolve().parent.parent.parent.parent / "src-tauri" / "binaries" / "backend",
        ...
        if d.exists():
            try:
                for item in d.iterdir():
                    if item.is_file() and (item.name.startswith("aria2-next-") or item.name.startswith("aria2c-next-")):
                        if sys.platform == "win32" and (item.name.endswith(".exe") or "windows" in item.name):
                            return item
                        elif sys.platform == "darwin" and ("macos" in item.name or "darwin" in item.name):
                            return item
            except Exception:
                pass
...
class DMAAria2cFD(Aria2cFD):
    ...
    def _call_process(self, cmd: List[Any], info_dict: Dict[str, Any]) -> tuple[str, str, int]:
        ...
        # ── 1. Allocate a free local port for aria2's RPC listener ─────────
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as _sock:
            _sock.bind(('127.0.0.1', 0))
            rpc_port = _sock.getsockname()[1]

        rpc_secret = f"dma{random.randint(0x100000, 0xFFFFFF):x}"
        rpc_url = f"http://127.0.0.1:{rpc_port}/jsonrpc"

        # ── 2. Minimal inline JSON-RPC 2.0 helper ──────────────────────────
        ...
        # ── 3. Rewrite cmd: strip stdout-only flags, inject RPC flags ───────
        STRIP_EXACT = {'--quiet', '-q'}
        STRIP_PREFIX = (
            '--console-log-level=', '--summary-interval=',
            '--enable-rpc', '--rpc-listen-port=', '--rpc-secret=',
        )
        filtered_cmd = []
        for arg in cmd:
        ...
        # Insert before the '--' option terminator
        try:
            idx = next(
                i for i, a in enumerate(filtered_cmd)
        ...
        enc = lambda v: v.encode('utf-8') if is_bytes else v
        ...
        # ── 4. Wait for RPC to become available (up to 5 s) ─────────────────
        rpc_ready = False
        for _ in range(50):
            if p.poll() is not None:
                break  # process exited before RPC came up (tiny file / error)
            try:
        ...
        if not rpc_ready:
            logger.warning("[aria2 RPC] Could not connect — no live progress for this stream")

        # ── 5. Poll RPC at ~100 ms for realtime progress ─────────────────────
        ...
        try:
                        ...
                        if active:
                            ...
                            for dl in active:
                                ...
                                eta: Optional[int] = (
                                    ...
                                    if speed > 0 and total > downloaded else None
                                )
                                self._hook_progress(
                                    {
                                        "status": "downloading",
                            ...
                            logger.info("[aria2 RPC] Active download finished. Shutting down daemon...")
                            try:
                                rpc("aria2.shutdown")
                            except Exception:
                                pass
            ...
            # ── 6. Emit a final "finished" hook once aria2c exits ─────────────
...
def download_video(
    ...
    use_aria2 = getattr(settings_data, "useAria2", True)
    
    # aria2 configurations
    aria2_max_connections = getattr(settings_data, "aria2MaxConnections", 16)
    aria2_concurrent_downloads = getattr(settings_data, "aria2ConcurrentDownloads", 5)
    aria2_split = getattr(settings_data, "aria2Split", 16)
    aria2_min_split_size = getattr(settings_data, "aria2MinSplitSize", "1M")
    aria2_preallocate = getattr(settings_data, "aria2Preallocate", True)
    aria2_check_certificate = getattr(settings_data, "aria2CheckCertificate", True)
    aria2_always_resume = getattr(settings_data, "aria2AlwaysResume", True)

    # yt-dlp customization options
    concurrent_fragment_downloads = getattr(settings_data, "concurrentFragmentDownloads", 4)
    download_retries = getattr(settings_data, "downloadRetries", 10)
    fragment_retries = getattr(settings_data, "fragmentRetries", 10)
    ffmpeg_location = getattr(settings_data, "ffmpegLocation", None)
    rate_limit = getattr(settings_data, "rateLimit", None)
    ...
    if any(lower_url.endswith(ext) for ext in [".m3u8", ".mpd"]) or "/manifest" in lower_url:
        is_stream = True

    job = jobs_registry.get_job(job_id)
    if job and job.formats and format_id:
        for fmt in job.formats:
            fmt_id = fmt.get("formatId") if isinstance(fmt, dict) else getattr(fmt, "formatId", None)
            if fmt_id == format_id:
                video_est = (fmt.get("videoEstSizeBytes") if isinstance(fmt, dict) else getattr(fmt, "videoEstSizeBytes", None)) or 0
                audio_est = (fmt.get("audioEstSizeBytes") if isinstance(fmt, dict) else getattr(fmt, "audioEstSizeBytes", None)) or 0
                is_stream = is_stream or (fmt.get("isStream") if isinstance(fmt, dict) else getattr(fmt, "isStream", False)) or False
                break
    ...
    # Clean the title of any invalid filename characters and escape percent signs for yt-dlp outtmpl
    clean_title = "video"
    if job and job.title:
        if is_chrome_intercept:
            # Preserve filename as-is for browser intercepted downloads
            clean_title = job.title.replace("%", "%%")
        else:
            # Remove control characters (0-31, 127)
            t = "".join(ch for ch in job.title if ord(ch) >= 32 and ord(ch) != 127)
            # Map forbidden characters to fullwidth counterparts
            mappings = {
                '/': '／',
                '\\': '＼',
                ':': '：',
            ...
            t = re.sub(r'\s+', " ", t)
            # Escape percent signs for template expansion
            t = t.replace("%", "%%")
            # Trim leading/trailing spaces, and only trailing dots (leading dots are legal)
            t = t.strip()
            while t.endswith('.'):
                t = t[:-1].strip()
            clean_title = t or "video"

    # Isolated temp template
    if skip_postprocessing:
        out_tmpl = os.path.join(str(app_temp_dir), clean_title)
    else:
    ...
    stream_state: Dict[str, Any] = {
        "phase": "single",           # 'single' | 'video' | 'audio'
        "video_done_bytes": 0,       # locked-in bytes once video stream finishes
        "video_total_bytes": 0,      # locked-in total once video stream finishes
    }
    ...
    def progress_hook(d: Dict[str, Any]):
        nonlocal combined_dl, combined_total
        if jobs_registry.is_paused(job_id) or jobs_registry.get_job(job_id) is None:
            logger.info(f"Pause or removal event detected in progress hook for job {job_id}")
        ...
        if filename:
            ...
            for ignored_ext in [".vtt", ".srt", ".ass", ".ssa", ".ttml", ".sbv", ".lrc", ".sub", ".idx", ".jpg", ".jpeg", ".png", ".webp", ".json", ".xml"]:
        ...
        if current_phase == "single" and detected == "video":
            ...
            current_phase = "audio"
        elif current_phase == "video" and status_str == "finished" and detected == "video":
        ...
        if current_phase == "video":
            vid_dl = downloaded
            vid_total = current_total if current_total > 0 else video_est
            aud_dl = 0
            aud_total = audio_est
            combined_dl = vid_dl
            combined_total = vid_total + aud_total if aud_total > 0 else vid_total
        elif current_phase == "audio":
            vid_dl = stream_state["video_done_bytes"] if stream_state["video_done_bytes"] > 0 else video_est
            vid_total = stream_state["video_total_bytes"] if stream_state["video_total_bytes"] > 0 else video_est
            aud_dl = downloaded
            aud_total = current_total if current_total > 0 else audio_est
            combined_dl = vid_dl + aud_dl
            combined_total = vid_total + aud_total
        else:
            # Single-stream download
            vid_dl = downloaded
            vid_total = current_total
            aud_dl = 0
            aud_total = 0
            combined_dl = downloaded
            combined_total = current_total
    ...
    if embed_subs and not skip_postprocessing:
        ...
        langs_str = getattr(settings_data, "subtitlesLangs", "all")
    ...
    if use_aria2 and not is_stream and aria2_bin is not None:
        opts["external_downloader"] = str(aria2_bin)
        
        aria2_args = [
            "-j", str(aria2_concurrent_downloads),
    ...
    try:
        try:
            ...
            if isinstance(first_err, DownloadPaused) or "DownloadPaused" in type(first_err).__name__:
                raise first_err
            if "cookiesfrombrowser" in active_opts:
                logger.warning(f"Download failed with native cookies ({browser}): {first_err}. Retrying without cookies...")
                active_opts.pop("cookiesfrombrowser", None)
                active_opts.pop("progress_hooks", None)
                active_opts.pop("postprocessor_hooks", None)
                with yt_dlp.YoutubeDL(active_opts) as ydl:
                    info = ydl.extract_info(url, download=True)
                    final_filepath = ydl.prepare_filename(info)
            else:
        ...
        logger.info(f"Download completed successfully. Saved and moved to: {dest_filepath} (resolution: {conflict_resolution})")
        ...
        try:
            ...
            logger.warning(f"Failed to clean up temp directory {app_temp_dir}: {cleanup_err}")
        ...
        if snapshot:
            ...
            if status == "failed":
                try:
                    if os.path.exists(app_temp_dir):
                        shutil.rmtree(app_temp_dir)
                        logger.info(f"Successfully cleaned up isolated temp folder: {app_temp_dir}")
                except Exception as rmtree_err:
                    logger.warning(f"Failed to clean up isolated temp folder {app_temp_dir}: {rmtree_err}")
